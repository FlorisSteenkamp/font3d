 ...
 */
function drawScene(
        glInfo: Required<GlDrawInfo>,
        projection: 'orthogonal' | 'perspective',
        effectsState: EffectsState) {

    const {
        gl, shaderInfo, buffers, triangleCount, texture, textureMetrics,
        xOffset, fontSize
    } = glInfo;
    ...
    const { modelViewMatrix, normalMatrix, projectionMatrix } =
        initializeRender(gl, projection, orientation, fontSize);

    const { attribLocations, uniformLocations } = shaderInfo;
    const {
        /*aVertexColor, */aVertexNormal, aVertexPosition, aWireframeCoord,
        aTextureCoord 
    } = attribLocations;

    const {
        uTextureRepeatCountX, uTextureRepeatCountY, uAmbientColor, uDirectionalColor, uDirectionalDirection,
        uModelViewMatrix, uNormalMatrix, uProjectionMatrix, uSampler,
        uWireframe, uTextureWidth, uXOffset, uVertexColor
    } = uniformLocations;
